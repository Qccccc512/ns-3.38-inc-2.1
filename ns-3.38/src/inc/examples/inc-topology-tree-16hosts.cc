/*
 * 在网计算协议 - 模拟测试：树状拓扑，15个交换机连接16个主机
 * 拓扑结构:
 *                          Switch 1 (根节点)
 *                        /                \
 *                Switch 2                Switch 3
...
NS_LOG_COMPONENT_DEFINE("IncTreeTopology16Hosts");
...
main(int argc, char* argv[])
{
  ...
  // 日志组件配置
  LogComponentEnable("IncTreeTopology16Hosts", LOG_LEVEL_INFO);
  ...
  // 创建31个节点：15个交换机和16个主机
  NodeContainer switchNodes;
  switchNodes.Create(15);  // Switch 1-15
  
  NodeContainer hostNodes;
  hostNodes.Create(16);    // Host 1-16
  ...
  // 存储所有的网络设备
  NetDeviceContainer devices[30]; // 15个交换机+16个主机之间的链路共30条
  
  // 交换机之间的链路 (共14条)
  int deviceIdx = 0;
  // 第一层到第二层 (1<->2, 1<->3)
  devices[deviceIdx++] = p2p.Install(switchNodes.Get(0), switchNodes.Get(1));
  devices[deviceIdx++] = p2p.Install(switchNodes.Get(0), switchNodes.Get(2));
  
  // 第二层到第三层 (2<->4, 2<->5, 3<->6, 3<->7)
  devices[deviceIdx++] = p2p.Install(switchNodes.Get(1), switchNodes.Get(3));
  devices[deviceIdx++] = p2p.Install(switchNodes.Get(1), switchNodes.Get(4));
  devices[deviceIdx++] = p2p.Install(switchNodes.Get(2), switchNodes.Get(5));
  devices[deviceIdx++] = p2p.Install(switchNodes.Get(2), switchNodes.Get(6));
  
  // 第三层到第四层 (4<->8, 4<->9, 5<->10, 5<->11, 6<->12, 6<->13, 7<->14, 7<->15)
  for (int i = 3; i < 7; i++) {
    for (int j = 0; j < 2; j++) {
      devices[deviceIdx++] = p2p.Install(switchNodes.Get(i), switchNodes.Get(2*i+1+j));
    }
  }
  ...
  for (int i = 7; i < 15; i++) {
    for (int j = 0; j < 2; j++) {
      devices[deviceIdx++] = p2p.Install(switchNodes.Get(i), hostNodes.Get(2*(i-7)+j));
    }
  }
  ...
  Ipv4InterfaceContainer interfaces[30];
  ...
  for (int i = 0; i < 15; i++) {
    ...
    if (i > 0) {
      ...
      } else if (parentIdx >= 1 && parentIdx <= 2) {
        interfaceIdx = 2 + (parentIdx - 1) * 2 + childPosition;  // 第二层到第三层的链路
      } else if (parentIdx >= 3 && parentIdx <= 6) {
        interfaceIdx = 6 + (parentIdx - 3) * 2 + childPosition;  // 第三层到第四层的链路
      }
    ...
    for (size_t j = 0; j < switchQPs[i].toChildren.size(); j++) {
      ...
      if (i == 0) {
        ...
        interfaceIdx = 2 + (i - 1) * 2 + j;  // 第二层到第三层的链路
      } else if (i >= 3 && i <= 6) {
        interfaceIdx = 6 + (i - 3) * 2 + j;  // 第三层到第四层的链路
      } else {
        interfaceIdx = 14 + (i - 7) * 2 + j;  // 第四层到主机的链路
      }
  ...
  for (int i = 0; i < 16; i++) {
    ...
    // 确定连接到主机的交换机编号
    int switchIdx = i / 2 + 7;  // 主机1-2连接到交换机8，主机3-4连接到交换机9，以此类推
    int hostPosition = i % 2;   // 0或1，表示是交换机的左或右子节点
    
    // 确定接口的索引
    int interfaceIdx = 14 + i;  // 第四层交换机到主机的链路从索引14开始
  ...
  // 设置Pcap跟踪
  p2p.EnablePcapAll("inc-topology-tree-16hosts", false);
